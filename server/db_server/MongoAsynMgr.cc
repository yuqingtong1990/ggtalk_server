...
MongoAsynMgr::MongoAsynMgr()
    : _loop(false), _sleep(0)
{}

MongoAsynMgr::~MongoAsynMgr()
{}

bool MongoAsynMgr::addConnect(const std::string &dbKey, ReadMode read_mode, const std::string &host_name,
                              const std::string &replset, const std::string &db_name, const std::string &username,
...
bool MongoAsynMgr::start() {
    ...
    _loop = true;
    _sleep = 10;
    _thread = std::thread(&MongoAsynMgr::run, this);
    return true;
}
