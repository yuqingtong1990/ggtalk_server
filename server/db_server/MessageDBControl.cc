...
void MessageDBControl::InsertUserMessage_callback(const ResultMap &ret,void* param) {
    for (auto&& iter = ret.begin(); iter != ret.end(); ++iter){
        LOG_INFO << "operId:" << iter->second->id << "code "<< iter->second->code <<"count" << iter->second->count;
    }
}

void MessageDBControl::InsertGroupMessage_callback(const ResultMap &ret,void* param) {
    for (auto&& iter = ret.begin(); iter != ret.end(); ++iter){
           LOG_INFO << "operId:" << iter->second->id << "code "<< iter->second->code <<"count" << iter->second->count;
    }
}

void MessageDBControl::InsertOfflineMsg_callback(const ResultMap &ret,void* param) {
    for (auto&& iter = ret.begin(); iter != ret.end(); ++iter){
            LOG_INFO << "operId:" << iter->second->id << "code "<< iter->second->code <<"count" << iter->second->count;
    }
}

void MessageDBControl::GetUserOffLineMsg_callback(const ResultMap &ret,void* param) {
     for (auto&& iter = ret.begin(); iter != ret.end(); ++iter){
         LOG_INFO << "operId:" << iter->second->id << "code "<< iter->second->code <<"count" << iter->second->count;
...
void MessageDBControl::SetMsgHaveRead_callback(const ResultMap &ret,void* param) {
     for (auto&& iter = ret.begin(); iter != ret.end(); ++iter) {
         LOG_INFO << "operId:" << iter->second->id << "code " << iter->second->code << "count" << iter->second->count;
