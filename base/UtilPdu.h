...
{
public:
	CByteStream(char* buf, uint32_t len);
	CByteStream(CSimpleBuffer* pSimpBuf, uint32_t pos);
	~CByteStream() {}

	char* GetBuf() { return m_pSimpBuf ? m_pSimpBuf->GetBuffer() : m_pBuf; }
	...
	void WriteData(char* data, uint32_t len);
	char* ReadData(uint32_t& len);
private:
