...
// 		strmem.append("\n-----END PUBLIC KEY-----\n");
		...
		do
		{
			...
			if ((bio = BIO_new_mem_buf(chPublicKey, -1)) == NULL)       //���ַ�����ȡRSA��Կ
				break;
			rsa = PEM_read_bio_RSAPublicKey(bio, NULL, NULL, NULL);   //��bio�ṹ�еõ�rsa�ṹ
			if (!rsa)
			{
	...
	RSA* GetRSAPrivateMem(std::string& strmem)
	{
		...
		do
		{
			...
			if ((bio = BIO_new_mem_buf(chPublicKey, -1)) == NULL)       //���ַ�����ȡRSA��Կ
				break;
			rsa = PEM_read_bio_RSAPrivateKey(bio, NULL, NULL, NULL);   //��bio�ṹ�еõ�rsa�ṹ
			if (!rsa)
			{
				ERR_load_crypto_strings();
				char errBuf[512] = { 0 };
	...
	void EncryptTEA(unsigned int *firstChunk, unsigned int *secondChunk, unsigned int* key)
	{
		...
		for (int i = 0; i < 8; i++)//8������(��Ҫ��Ӧ����Ľ��ܺ��ĺ���������һ��)
		{
			sum += delta;
